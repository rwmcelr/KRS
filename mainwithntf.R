...
library(pheatmap)
library(gplots)
# Declare list of NM23H2 related transcription factors
ntf <- read.csv("nme2tf.csv")
...
sigGenes <- function(name, plots=T, filBy="padj", filVal=0.05, con1, con2) {
  ...
  fk <- as.data.frame(res)
  fk$important <- row.names(res)
  ntfFinal <- merge(x = fk, y = ntf, by = "symbol", all.y = TRUE, row.names = TRUE)
  write.csv(ntfFinal, file="ntfFinal.csv")
  ...
  if (plots) {
    ...
    resMA <- lfcShrink(dds=dds, contrast=c("condition",con1,con2), type="ashr")
    png(file=paste0(name,"_MAplot_Shrink.png"),width=768,height=768)
    plotMA(resMA, ylim=c(-5,5), colSig = "red", main=name)
    dev.off()
